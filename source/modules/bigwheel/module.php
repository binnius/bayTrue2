...
class BigwheelModule extends WeModule {
	...
	public function doManage() {
		...
		if (!empty($list)) {
			foreach($list as &$item) {
				...
				$bigwheel = pdo_fetch("SELECT fansnum, viewnum,starttime,endtime,isshow,is_default FROM ".tablename('bigwheel_reply')." WHERE rid = :rid ", array(':rid' => $item['id']));
	...
	public function doaddcard(){
		...
		if($_GPC['action']=='save'){
			...
			$temp = pdo_insert('bigwheel_reply', $insert);
			if($temp==false){
				$this->message('抱歉，刚才添加的数据失败！','', -1);              
			}else{
				$this->message('大转盘添加数据成功！', create_url('site/module/manage', array('name' => 'bigwheel')), 0);      
			}		
		}
	...
	public function doUpdateCard (){
		...
		if($_GPC['action']=='update'){
			...
			$insert['starttime']=strtotime($starttime);
			$insert['endtime']=strtotime($endtime);
			$temp = pdo_update('bigwheel_reply', $insert,array('id'=>$id,'rid'=>$rid));
			if($temp==false){
				$this->message('抱歉，刚才修改的数据失败！','', -1);              
			}else{
				$this->message('大转盘修改数据成功！', create_url('site/module/manage',array('name' => 'bigwheel')), 0);      
			}		
		} 
	...
	public function dosetshow(){
		...
		$temp = pdo_update('bigwheel_reply',array('isshow'=>$_GPC['isshow']),array('rid'=>$rid));
		if($temp==false){
	...
	public function dochangedefault(){
 		global $_GPC;
		pdo_update('bigwheel_reply',array('is_default'=>$_GPC['ck']),array('rid'=>$_GPC['id']));
 	}
	
	public function setkeyword($_kewords,$rid=0,$_module='bigwheel'){
		global $_W;
		if($rid==0){
