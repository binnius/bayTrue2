...
class BigwheelModuleSite extends WeModuleSite {
	...
	public function doMobileindex() {
		...
		//录入bigwheel_fans数据
 		if(empty($id)){
			$reply=pdo_fetch ("SELECT * FROM ".tablename("rule")." WHERE weid = :weid  AND module='bigwheel' ORDER BY `is_default` DESC limit 1", array(':weid' => $_W['weid']));
			$id=$reply['id'];
 		}else{
		...
		if($reply['most_num_times']>0 && $reply['number_times']>0){
			$detail='本次活动共可以转'.$reply['number_times'].'次，每天可以转 '.$reply['most_num_times'].' 次卡! 你共已经转了 <span id="totalcount">'.intval($fans['totalnum']).'</span> 次 ，今天转了<span id="count">'.intval($fans['todaynum']).'</span> 次.';
	...
	public function doMobilegetaward(){
		...
		foreach($cArr as $k=>$c){
			if($rand<$reply['c_num_'.$c]){
				...
				//增加一个中奖人数，添加中奖信息
				$sn=random(16);
				$temp = pdo_update('bigwheel_reply',array('c_draw_'.$c=>$reply['c_draw_'.$c]+1),array('id'=>$reply['id']));
				if($temp==false){
